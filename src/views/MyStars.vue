...
onMounted(async () => {
  ...
  loading.value = true
  starredSkills.value = await store.fetchStarredSkills(auth.user!.id)
  loading.value = false
})
...
<template>
  <div class="page-container">
    ...
    <div v-if="!auth.isLoggedIn && !auth.loading" class="auth-prompt">
      <StarIcon :size="40" class="prompt-icon" />
      <h2>登录后查看你的收藏</h2>
      <p>收藏你喜欢的技能，随时回来查看</p>
    ...
    <template v-else>
      ...
      <div class="page-header">
        ...
        <p class="page-subtitle">共 {{ starredSkills.length }} 个技能</p>
      </div>

      <!-- 有数据 -->
      <div v-if="starredSkills.length > 0" class="stars-grid">
...
.prompt-icon { color: var(--color-star); margin-bottom: 16px; }
...
.empty-state h3 { font-size: 17px; font-weight: 600; color: var(--color-text); margin-top: 16px; margin-bottom: 6px; }
...
.browse-btn:hover { background: var(--color-primary-hover); }
</style>
